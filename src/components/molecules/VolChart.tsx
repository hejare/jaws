    ...
    const fetchData = async () => {
      ...
      while (i < navStats.length - volatilityPeriod) {
        const volatility = calculateVolatility(
          navStats.slice(i, volatilityPeriod + i).map((jawsStat) => jawsStat.close),
          volatilityPeriod
        );
        const date = new Date(Date.parse(navStats[volatilityPeriod + i].date)).toISOString().split("T")[0];
        jawsVolStats.push({
          volatility,
          date,
        });
      ...
      const { spy, vthr, qqq } = Object.entries(await getTickerBars({
        ...
        while (i < tickerStats.length - volatilityPeriod) {
          const volatility = calculateVolatility(tickerStats.slice(i, volatilityPeriod + i).map((indexStat) => indexStat.c), volatilityPeriod);
          const date = new Date(Date.parse(tickerStats[volatilityPeriod + i].t)).toISOString().split("T")[0];
          acc[accTicker].push({
            volatility,
            date,
          });
      ...
      // filter out dates that are bigger than the last date of all the indexes
        .filter(navStats => {
          return Date.parse(navStats.date) > Date.parse(spy[0].date) && 
    ...
    if (active && payload && payload.length && payload[0].value) {
      return (
        <div style={{ fontSize: "0.7em", backgroundColor: "#000", padding: "2px 15px", borderRadius: 20 }}>
          <div>{label}</div>
          <div style={{ color: "#71b16b" }}>Jaws: {payload[0].value.toFixed(4)}</div>
